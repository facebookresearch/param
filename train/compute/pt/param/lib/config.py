...
class BenchmarkConfig:
    ...
    # Returns all ops in the config
    def get_op_configs(self):
        def make_op_config(op_name: str):
        ...
        for op_name in self.bench_config:
            op_config = make_op_config(op_name)
            if op_config is not None:
                self.op_configs.append(op_config)

        return self.op_configs
...
class OperatorConfig():
    ...
    @property
    def build_iterator(self) -> ConfigIterator:
        return self._build_iterator

    @build_iterator.setter
    def build_iterator(self, value: ConfigIterator):
        self._build_iterator = value

    @property
    def input_iterator(self) -> ConfigIterator:
        return self._input_iterator

    @input_iterator.setter
    def input_iterator(self, value: ConfigIterator):
    ...
    def build_data_generator(self, value: DataGenerator):
    ...
    def input_data_generator(self, value: DataGenerator):
