...
def run_ncu(args:str, device:str, metrics:str, out_prefix:str):
    ...
    if not ncu_bin:
        ncu_bin = NCU_BIN
    ncu_options = f"--log-file {out_prefix}.ncu.log --csv --target-processes all " \
                  f"--metrics {metrics} --nvtx --nvtx-include op_bench/+"

    cmd = [ncu_bin] + ncu_options.split(" ") + args.split(" ") + ["--device", device, "--out_json", f"{out_prefix}.json"]
...
def main():
    ...
    parser.add_argument(
        "--bench_args", type=str, required=True, help="Args to pass to benchmark script."
        )
    parser.add_argument(
        "--device", type=str, default="cuda", help="The target device."
        )
    parser.add_argument(
        "--metrics", type=str, default="", help="The  config file."
        )
    parser.add_argument(
    ...
    if args.metrics_file:
        with open(args.metrics_file, 'r') as metrics_file:
            metrics.update({x.strip() for x in metrics_file.read().split(",") if x.strip()})
    # combine all metrics
