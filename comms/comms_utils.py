...
def parsesize(ipValue):
    ...
    # print("\t units: %d value: %s size: %.3f " % (units, value, size))
    return int(size)


def getAlgBW(elapsedTimeNS, dataSize, numIters):
    avgIterNS = 0.0
    # print("\t elapsedTimeNS: %d dataSize: %d numIters: %d " % (elapsedTimeNS, dataSize, numIters))
    if numIters != 0:
...
class comms_world_info_holder:
    def __init__(self, master_ip, master_port, num_tpu_cores, mpi_env_params):
        ...
        # self.num_tpu_cores = args.num_tpu_cores
...
class ads_feeds_shard:
    def __init__(self):
        pass

    # To initialize following elements
    # ln_emb, num_sparse_fea, n_emb_per_rank, local_emb_slice, dims_sum_per_rank
