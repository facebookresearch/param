...
def trace_generate_lru(
    ...
    max_sd = list_sd[-1]
    num_line_access = len(line_accesses)
    i = 0
    ztrace = []
    for _ in range(out_trace_len):
        ...
        if sd == 0:  # new reference #
            ...
            line_ref = line_accesses[num_line_access - sd]
            mem_ref = np.uint64(line_ref * cache_line_size + mem_ref_within_line)
            line_accesses.pop(num_line_access - sd)
            line_accesses.append(line_ref)
...
def trace_generate_rand(
    ...
    max_sd = list_sd[-1]
    num_line_access = len(line_accesses)  # !!!Unique,
    i = 0
    ztrace = []
    for _ in range(out_trace_len):
        sd = generate_stack_distance(list_sd, cumm_sd, max_sd, i, enable_padding)
...
def trace_profile(trace, enable_padding=False):
    ...
    for x in trace:
        ...
        try:  # found #
            ...
            rstack.insert(len_rstack, r)

    if enable_padding:
        # WARNING: notice that as the ratio between the number of samples (len_stack_dist)
        ...
        # average number of samples len_stack_dist/c artificially.
        len_stack_dist = len(stack_distances)
        c = max(stack_distances)
        padding = int(np.ceil(len_stack_dist / c))
...
if __name__ == "__main__":
    ...
    dist_sd = list(
        map(lambda tuple_x_k: tuple_x_k[1] / float(len_stack_dist), dc)
    )  # k = tuple_x_k[1]
    cumm_sd = []  # np.cumsum(dc).tolist() #prefixsum
    for i, (_, k) in enumerate(dc):
        if i == 0:
            cumm_sd.append(k / float(len_stack_dist))
        else:
            # add the 2nd element of the i-th tuple in the dist_sd list
            cumm_sd.append(cumm_sd[i - 1] + (k / float(len_stack_dist)))
