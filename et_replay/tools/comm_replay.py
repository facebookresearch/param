...
from param.comm.comm_utils import (
...
class commsTraceReplayBench(paramCommsBench):
    ...
    def benchTime(self, commsParams: commsParamsHolderBase) -> None:
        ...
        if commsParams.enable_profiler:
            ...
            self.collectiveArgs.enable_profiler = comm_utils.startProfiler(
        ...
        if self.collectiveArgs.enable_profiler:
            comm_utils.sampleProfiler(stop=True)
    ...
    def initBackend(
        ...
        if commsParams.nw_stack == "pytorch-dist":
            from param.comm.backend.pytorch_dist import (
                PyTorchDistBackend,
            )

            self.backendFuncs = PyTorchDistBackend(bootstrap_info, commsParams)
        elif commsParams.nw_stack == "pytorch-xla-tpu":
            from param.comm.backend.pytorch_tpu import PyTorchTPUBackend
    ...
    def initBench(
        ...
        if args.output_ranks == "all":
            ...
            self.outputRanks = comm_utils.parseRankList(args.output_ranks)
    ...
    def readRawTrace(self, remotePath: str, rank: int) -> None:
        ...
        if self.use_remote_trace:
            ...
            if protocol in ("http", "https", "ftp"):
                raw_comms_trace = comm_utils.commonUrlRead(remotePath=remotePath)
            else:
                try:
                    from param.comm.fb.internals import (
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="param.comm Trace Replay Mode",
