...
from param.compute.python.lib import pytorch as lib_pytorch
from param.compute.python.lib.init_helper import load_modules
...
from param.execution_trace import (
    ExecutionTrace,
    NodeType,
)

from param.compute.python.tools.utility import trace_handler
from param.compute.python.workloads import pytorch as workloads_pytorch
...
class ExgrReplayManager:
    ...
    def extract_subgraph(self, root):
        ...
        def anlayze_node(node):
            ...
            for _, t_id, _ in get_input_tensors(node):
                if self.tensor_with_device:
                    t_id = tuple(list(t_id)[:5])
                if node.name == "record_param.comm" and (
    ...
    def generate_code(self):
        ...
        def _generate_run_ops_str(override):
            ...
            start_special = False
            for node in self.sorted_nodes:
                if node.name == "record_param.comm" and not self.compute_only:
    ...
    def init_comms(self):
        ...
        time.sleep(1)
        self.bootstrap_info = bootstrap_info_holder(
